...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Проверяем, существует ли ключ 'kuzov' в массиве
    if (isset($data['model'])) {
        // Получаем значение, переданное из JavaScript
        $selectedValue = $data['model'];

        $arr_model = aj_get("select kuzov from main where model_name='" . $selectedValue . "' group by kuzov order by kuzov");
        // Отправляем ответ обратно в JavaScript
        echo json_encode(["status" => "success", "message" => "Значение получено", "kuzov" =>  $arr_model]);
    } else {
        // Отправляем ошибку, если ключ 'marka' не найден
        echo json_encode(["status" => "error", "message" => "ключ 'kuzov' не найден"]);
    }
} else {
